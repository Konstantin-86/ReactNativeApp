...
const Examen = () => {
  ...
  useEffect(() => {
    const getCurrentProf = async () => {
      try {
        ...
        if (savedProf) {
          setCurrentProf(allProf.find((item) => item.nameOfProf === currentProf.nameOfProf) || allProf[0]);
        }
      } catch (error) {
  ...
  return (
    <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
      ...
      <Pressable style={styles.myBtn} onPress={() => setCountOfQuestions(countOfQuestions + 1)}>
        <Text>Next</Text>
      </Pressable>
    </View>
  );
